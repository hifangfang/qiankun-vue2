...
import {getUrlParam,getQueryString} from "@/utils/js/urlpath-utils";
...
export class LocalStorageUtil {
  static getItem(key) {
    if (MICRO_CONFIG.isLocalStorageSubGuidFilter&&(isView||isBilogin)) {
      try {
        let subGuid = this.getSubGuid();
        if (subGuid) {
          console.log('subGuid:' + subGuid);
        } else {
          console.log('subGuid未获取到');
        }
        let appObjectStr = localforage.getItem("appObject");
        if (appObjectStr) {
    ...
    if (MICRO_CONFIG.isLocalStorageSubGuidFilter&&(isView||isBilogin)) {
      try {
        ...
        if (appObjectStr) {
          ...
          localforage.setItem('appObject', JSON.stringify(appObject));
        } else {
          let appObject = {};
          let appinfo = {};
          appinfo[key] = val;
  ...
  static removeAppObjectItem(key) {
    if (MICRO_CONFIG.isLocalStorageSubGuidFilter) {
      try {
        ...
        if (appObjectStr) {
          ...
          localforage.setItem("appObject", JSON.stringify(appObjectStr));
        }
      } catch (error) {
  ...
  static getSubGuid() {
    try {
      //先从url中取
      let subGuid = getUrlParam('subGuid');
      if (!subGuid) {
  ...
  static getHrefQueryData(){
    ...
    if (t !== '' && t !== null) {
      ...
      if(href.indexOf('/bi/') >=0 || href.indexOf('/bilogin') >=0){
        ...
        if (haveToken.startsWith("bearer")) {
          window.localforage.setItem("X-Gisq-Token", t);
        } else {
          window.localforage.setItem("X-Gisq-Token", "Bearer " + t);
        }
        window.localforage.setItem('loginflag', 'true');
        // @ts-ignore
        let Base64 = require("js-base64").Base64;
        let tokensplit = t.split(".");
        if (tokensplit.length > 1) {
          let a = Base64.decode(t.split(".")[1]);
          let loginname = JSON.parse(a).user_name;
          window.localforage.setItem('username', loginname);
          let userId = JSON.parse(a).user_id;
          window.localforage.setItem('userId', userId);
        }
      }
      // next(from.fullPath)
    }
