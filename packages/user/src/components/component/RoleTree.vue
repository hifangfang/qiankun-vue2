<template>
  <div class="role-tree">
    <el-dialog :visible.sync="visible"  :title="title" :before-close="close" :close-on-click-modal="false">
...
<script lang="ts">
  ...
  export default class roleTree extends Vue {
    ...
    subGuid;
    visible = false;
    subsystem = [] as RoleSelectTreeVo[];//编号配置已存在的子系统
    defaultProps = {
      label: 'name',
      children: 'children',
      isLeaf: 'leaf',
    ...
    loadNode(node, resolve) {
      ...
      setTimeout(async() => {
        const reponse = await RolemgrApi.getRoleSelectTree(groupid,type,subGuid)
        if(node.level === 0){
          this.subsystem =reponse;
        }
        resolve(reponse);
      }, 500);
    }
